...
export default function BigRoundedLinkBtn({upperSmallText, bigBtmText, href, className}: {upperSmallText:string, bigBtmText: string, href:string, className?:string}) {
  return (
    <Link
      to={href}
      className={`w-full max-w-[336px] px-8 py-4 rounded-full flex flex-col items-center hover:shadow-lg hover:scale-105 transition-transform ease-in ${className}`}
    >
      <span className="text-sm sm:text-base font-medium">{upperSmallText}</span>
